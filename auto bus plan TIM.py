...
#x = dict()
#for voertuig in aantal_voertuigen:
#    for dienst in aantal_diensten:
#        x[voertuig, dienst] = m.add_var(var_type=BINARY)
...
#for j in range(aantal_diensten):
#    m += xsum(x[i][j] for i in range(aantal_voertuigen))

# Constraint 5: A vehicle must be available to be assigned to a service
#for i in range(aantal_voertuigen):
#    m += xsum(x[i][j] for j in range(aantal_diensten)) <= 5 * voertuig.loc[i, 'beschikbaar'] # TODO replace 5
