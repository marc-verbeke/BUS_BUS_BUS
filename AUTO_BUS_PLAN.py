...
diensten = pd.read_excel(excel_file_path)
excel_file_path = "data/test_voertuigen.xlsx"
voertuigen = pd.read_excel(excel_file_path)
...
# elk voertuig wordt aan maximaal één rit toegewezen
...
for j in range(aantal_diensten):
    for i in range(aantal_voertuigen):
        if x[i][j].x >= 0.99:
            ...
            print(f"{diensten_nummer[j]} door {voertuigen_nummer[i]}")
